...
async function buildWeb(domain, expoPublicReplId) {
  ...
  return new Promise((resolve) => {
    const env = {
      ...
      EXPO_PUBLIC_API_BASE: `https://${domain}`,
    };
    if (expoPublicReplId) env.EXPO_PUBLIC_REPL_ID = expoPublicReplId;
    ...
    webProcess.on("close", (code) => {
      if (code === 0) {
        ...
        console.warn(`Web build exited with code ${code} — web preview unavailable`);
      }
      resolve(); // non-fatal: mobile build still succeeds
    });
...
async function main() {
  ...
  // Build web SPA — non-fatal; mobile Expo Go build is already complete
