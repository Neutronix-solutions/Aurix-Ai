...
const PENDING_REG_TTL_MS = 30 * 60 * 1000;
...
async function sendOtpViaTwilio(phone: string, otp: string): Promise<boolean> {
  ...
  try {
    ...
    const body = new URLSearchParams({
      To: phone,
      From: from,
      Body: `Aurix AI verification code: ${otp}. Valid for 5 minutes.`,
    });
...
async function sendOtpViaEmail(
  ...
  const from =
    ...
    "Aurix AI <noreply@neutronixs.com>";

  const safeLog = log ?? console;

  if (!apiKey) {
    safeLog.warn({ to: toEmail, from }, "RESEND_API_KEY is missing; email OTP not sent");
    return false;
  }

  const subject =
    purpose === "register"
      ? "Verify your Aurix AI account"
      : "Confirm your new email — Aurix AI";
  ...
  try {
    ...
    if (!res.ok) {
      ...
      safeLog.warn({ status: res.status, body, to: toEmail, from }, "Resend API rejected email");
      return false;
    }

    return true;
  } catch (err) {
    safeLog.error({ err, to: toEmail, from }, "Resend API call threw");
    return false;
  }
}
...
setInterval(() => {
  ...
  for (const [k, v] of pendingRegistrations) {
    if (now > v.expiresAt + PENDING_REG_TTL_MS) pendingRegistrations.delete(k);
  }

  for (const [k, v] of phoneOtpStore) {
    if (now > v.expiresAt + OTP_TTL_MS) phoneOtpStore.delete(k);
  }

  for (const [k, v] of emailOtpStore) {
    if (now > v.expiresAt + OTP_TTL_MS) emailOtpStore.delete(k);
  }
}, 5 * 60 * 1000).unref();
  ...
  try {
    ...
    const payload: Record<string, unknown> = {
      ...
      message: sent
        ...
        : resendConfigured
          ? "Email could not be delivered. Check Resend domain/sender settings."
          : "Verification code ready. RESEND_API_KEY is not set.",
    };
    ...
    req.log?.error?.({ err }, "register/start failed");
...
router.post("/auth/register/verify", async (req, res) => {
  try {
    ...
    if (pending.otp !== String(otp).trim()) {
      pending.attempts += 1;
      res.status(400).json({
        error: `Incorrect code. ${OTP_MAX_VERIFY_ATTEMPTS - pending.attempts} attempts remaining.`,
      });
      return;
    }
    ...
    req.log?.error?.({ err }, "register/verify failed");
...
router.post("/auth/register/resend", async (req, res) => {
  try {
    ...
    req.log?.error?.({ err }, "register/resend failed");
...
router.post("/auth/register", registerRateLimit, async (req, res) => {
  try {
    ...
    const message = err instanceof Error ? err.message : String(err);
    console.error("[register/direct] failed:", err);
    req.log?.error?.({ err }, "register/direct failed");
...
router.put("/auth/me/settings", requireAuth, async (req, res) => {
  try {
    const { userId } = (req as Request & { user: AuthPayload }).user;
    const { monthlyIncome, language, name, currency } = req.body as {
    ...
    req.log?.error?.({ err }, "settings failed");
...
router.patch("/auth/me/profile", requireAuth, async (req, res) => {
  try {
    ...
    req.log?.error?.({ err }, "profile failed");
...
router.post("/auth/me/password", requireAuth, async (req, res) => {
  try {
    ...
    const message = err instanceof Error ? err.message : String(err);
    console.error("[password/post] failed:", err);
    req.log?.error?.({ err }, "password/post failed");
...
router.post("/auth/me/avatar", requireAuth, async (req, res) => {
  try {
    ...
    req.log?.error?.({ err }, "avatar failed");
...
router.post("/auth/me/email/change-request", requireAuth, async (req, res) => {
  try {
    const { userId } = (req as Request & { user: AuthPayload }).user;
    const { newEmail: rawNew, password } = req.body as {
      newEmail: string;
      password?: string;
    };

    if (!rawNew || !isValidEmail(rawNew)) {
      res.status(400).json({ error: "Please enter a valid email address" });
      return;
    }
    ...
    if (!password) {
      res.status(400).json({ error: "Please confirm your current password" });
      return;
    }
    ...
    const payload: Record<string, unknown> = {
      success: true,
      message: sent ? "Verification code sent to new email" : "Verification code generated",
    };
    ...
    req.log?.error?.({ err }, "email/change-request failed");
...
router.post("/auth/me/email/change-verify", requireAuth, async (req, res) => {
  try {
    ...
    if (!me?.pendingEmail) {
      res.status(400).json({ error: "No pending email change. Request one first." });
      return;
    }
    ...
    if (!stored) {
      res.status(400).json({ error: "Code expired or not found. Please resend." });
      return;
    }

    if (Date.now() > stored.expiresAt) {
      emailOtpStore.delete(me.pendingEmail);
      res.status(400).json({ error: "Code expired. Please resend." });
      return;
    }
    ...
    if (stored.otp !== String(otp).trim()) {
      stored.attempts += 1;
      res.status(400).json({
        error: `Incorrect code. ${OTP_MAX_VERIFY_ATTEMPTS - stored.attempts} attempts remaining.`,
      });
      return;
    }

    const taken = await db.select().from(users)
      .where(and(eq(users.email, me.pendingEmail), ne(users.id, userId)))
      .limit(1);
    ...
    req.log?.error?.({ err }, "email/change-verify failed");
...
router.post("/auth/send-otp", requireAuth, async (req, res) => {
  try {
    ...
    if (isDev()) {
      res.json({
        success: true,
        message: "OTP ready. Twilio is not configured.",
        devOtp: otp,
      });
      return;
    }

    req.log.error({ phone }, "OTP delivery failed in production");
...
router.post("/auth/verify-otp", requireAuth, async (req, res) => {
  try {
    ...
    if (!user?.phoneNumber) {
      res.status(400).json({ error: "No phone number on file. Send OTP first." });
      return;
    }
    ...
    if (!stored) {
      res.status(400).json({ error: "OTP expired or not found. Please resend." });
      return;
    }

    if (Date.now() > stored.expiresAt) {
      phoneOtpStore.delete(user.phoneNumber);
      res.status(400).json({ error: "OTP expired. Please resend." });
      return;
    }
    ...
    if (stored.otp !== String(otp).trim()) {
      stored.attempts += 1;
      res.status(400).json({
        error: `Incorrect OTP. ${OTP_MAX_VERIFY_ATTEMPTS - stored.attempts} attempts remaining.`,
      });
      return;
    }
...
router.post("/auth/send-email-otp", requireAuth, async (req, res) => {
  try {
    ...
    if (attemptRec.lockedUntil && Date.now() < attemptRec.lockedUntil) {
      ...
      res.status(429).json({
        error: `Too many failed attempts. Please try again in ${waitMin} minute(s).`,
      });
      return;
    }
    ...
    if (isDev()) {
      res.json({
        success: true,
        message: "OTP ready. Email service is not configured.",
        devOtp: otp,
      });
      return;
    }

    req.log.error({ email: user.email }, "Email OTP delivery failed");
    ...
    req.log?.error?.({ err }, "send-email-otp failed");
  ...
  try {
    ...
    if (stored.otp !== otp.trim()) {
      ...
      if (updated.lockedUntil) {
        await otpDelete(user.email);
        res.status(429).json({
          error: "Too many incorrect attempts. Your account is temporarily locked for 24 hours.",
        });
      } else {
    ...
    req.log?.error?.({ err }, "verify-email-otp failed");
...
router.patch("/auth/me/password", requireAuth, async (req, res) => {
  try {
    ...
    const message = err instanceof Error ? err.message : String(err);
    console.error("[password/patch] failed:", err);
    req.log?.error?.({ err }, "password/patch failed");
